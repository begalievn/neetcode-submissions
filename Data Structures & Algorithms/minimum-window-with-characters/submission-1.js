class Solution {
    ...
    minWindow(s, t) {
        ...
        for (let r = 0; r < s.length; r++) {
            ...
            console.log('upper', { l, r });

            while (this.checkMaps(freqMap, need)) {
                console.log('inner', { l, r, res });
                if (res.length == 0) {
                    console.log('first assignment', [l, r]);
                    res = [l, r];
                } else {
                    const cur = r - l + 1;
                    console.log('conditions met', { cur, res})
                    if (cur < (res[1] - res[0] + 1)) {
                        console.log('assigning', { l, r });
                        res[0] = l;
                        res[1] = r;
                    }
                }
        ...
        if (res[1] >= s.length) return s.substring(res[0]);
